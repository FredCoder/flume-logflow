...
public class ReliableTaildirEventReader implements ReliableEventReader {
    ...
    private ReliableTaildirEventReader(Map<String, String> filePaths,
                                       ...
                                       boolean annotateFileName, String fileNameHeader, String linePrefix) throws IOException {
        // Sanity checks
            ...
            while (jr.hasNext()) {
                ...
                while (jr.hasNext()) {
                    switch (jr.nextName()) {
                        ...
                        case "file":
                            ...
                            break;
                        case "lineNumber":
                            lineNumber = Long.parseLong(jr.nextString());
                            break;
                    }
                }
                jr.endObject();
                ...
                for (Object v : Arrays.asList(inode, pos, path, lineNumber)) {
                    Preconditions.checkNotNull(v, "Detected missing value in position file. "
                            + "inode: " + inode + ", pos: " + pos + ", path: " + path + ", lineNumber: " + lineNumber);
                }
                TailFile tf = tailFiles.get(inode);
                if (tf != null && tf.updatePos(path, inode, pos, lineNumber)) {
        ...
        for (TaildirMatcher taildir : taildirCache) {
            ...
            for (File f : taildir.getMatchingFiles()) {
                ...
                if (tf == null || !tf.getPath().equals(f.getAbsolutePath())) {
                    ...
                    // 读过的文件
                    // 判断文件是否更新的依据
                    // 是否需要更新 最近文件发生了变化，但是最近没采集
                    boolean updated = tf.getLastUpdated() < f.lastModified();
                    if (updated) {
                        // 读文件句柄关闭，重新打开 pos 为上次读取位置
                        if (tf.getRaf() == null) {
//                            tf = openFile(f, headers, inode, tf.getPos());
                            tf = openFile(f, headers, inode, tf.getPos(), tf.getLineNumber());
                        }
                        if (f.length() < tf.getPos()) {
                            // 可能是文件清空重新更新的情况，从头开始读
                            // note: 场景预设为文件 只可 append 或者清空重写
    ...
    private TailFile openFile(File file, Map<String, String> headers, long inode, long pos) {
        try {
            ...
            return new TailFile(file, headers, inode, pos, this.linePrefix);
        } catch (IOException e) {
            throw new FlumeException("Failed opening file: " + file, e);
        }
    }

    private TailFile openFile(File file, Map<String, String> headers, long inode, long pos, long lineNumber) {
        try {
            logger.info("Opening file: " + file + ", inode: " + inode + ", pos: " + pos + ", lineNumber: " + lineNumber);
            return new TailFile(file, headers, inode, pos, lineNumber, this.linePrefix);
        } catch (IOException e) {
            throw new FlumeException("Failed opening file: " + file, e);
        }
    }
    ...
    public static class Builder {
        ...
        public Builder linePrefix(String linePrefix) {
            this.linePrefix = linePrefix;
            return this;
        }
        ...
        public ReliableTaildirEventReader build() throws IOException {
            return new ReliableTaildirEventReader(filePaths, headerTable, positionFilePath, skipToEnd,
                    ...
                    annotateFileName, fileNameHeader, linePrefix);
        }
    }
